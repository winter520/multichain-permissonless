 ...
 */
 async function fetchChunk1(
  ...
  try {
    ...
    const callData = await useBatchData({
      chainId,
      calls: chunk.map(obj => ({target: obj.address, callData: obj.callData}))
    })
    // console.log(callData)
    if (callData?.returnData && callData?.blockNumber) {
...
export function activeListeningKeys(
  allListeners: AppState['evm']['callListeners'],
  chainId?: number
...
export function outdatedListeningKeys(
  callResults: AppState['evm']['callResults'],
...
export default function Updater({type}: {type?:number}): null {
  ...
  const state = useSelector<AppState, AppState['evm']>(state => state.evm)
  ...
  const updateResults = useCallback(() => {
    ...
    cancellations.current = {
      ...
      cancellations: chunkedCalls.map((chunk, index) => {
        ...
        // const { cancel, promise } = retry(() => chainId && useChainId && Number(useChainId) !== Number(chainId) ? fetchChunk1(useChainId, chunk, latestBlockNumber) : fetchChunk(multicallContract, chunk, latestBlockNumber), {
        const { cancel, promise } = retry(() => fetchChunk1(useChainId, chunk, latestBlockNumber), {
          n: Infinity,
          minWait: 2500,
          maxWait: 3500
        })
